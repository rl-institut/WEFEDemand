...
def plotly_df(
    ...
    if legend:  # Check if legend is passed
        if not len(legend) == df.shape[1]:  # check if legend is passed for every column
            raise Exception(
                "Length of passed legend list does not match number of columns in passed df"
            )
...
def plot_timeseries_segments(
    ...
    if group_dur == "day":
        ...
        mask = (data_pivot.index > pd.to_datetime("2019-" + start_date)).strftime(
            "%U"
...
def plot_timeseries_heatmaps(
    df, column, group_dur="day", wide=False, zmin=None, zmax=None
):
    """
