...
service_water_use_cases_list = ramp_control.generate_service_water_use_cases(
    service_water_dict, admin_input
)
agro_processing_use_cases_list = ramp_control.generate_agro_processing_use_cases(
    agro_processing_dict, admin_input
)
elec_use_cases_list = ramp_control.generate_electric_appliances_use_cases(
    households_dict, admin_input
)
cooking_use_cases_list = ramp_control.generate_cooking_demand_use_cases(
    cooking_demand_dict, admin_input
)
drinking_water_use_cases_list = ramp_control.generate_drinking_water_use_cases(
    drinking_water_dict, admin_input
)

# %%
# Run load use_cases and create demand profiles
service_water_dp = ramp_control.run_use_cases(
    service_water_use_cases_list, service_water_dict, "Service water"
)
drinking_water_dp = ramp_control.run_use_cases(
    drinking_water_use_cases_list, drinking_water_dict, "Drinking water"
)
elec_lp = ramp_control.run_use_cases(
    elec_use_cases_list, households_dict, "Household appliances"
)
cooking_dp = ramp_control.run_use_cases(
    cooking_use_cases_list, cooking_demand_dict, "Cooking demand"
)
agro_processing_dp = ramp_control.run_use_cases(
    agro_processing_use_cases_list, agro_processing_dict, "Agro-processing"
)
...
all_demands_day = all_demands_df.groupby(
    all_demands_df.index.strftime("%H"), sort=False
).mean()

all_demands_week = all_demands_df.groupby(
    all_demands_df.index.strftime("%a - %H"), sort=False
).mean()
...
fig = plotting.plotly_high_res_df(
    fig, df=drinking_water_dp.resample("h").sum(), subplot_row=2, prefix="h_"
)
fig = plotting.plotly_high_res_df(fig, df=service_water_dp, subplot_row=3, prefix=" ")
fig = plotting.plotly_high_res_df(
    fig, df=service_water_dp.resample("h").sum(), subplot_row=4, prefix="h_"
)
