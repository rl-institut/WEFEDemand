class RelationalDf:
    def __init__(self, table_id, table_name, df):
        ...
        self.child_tables = (
            {}
        )  # dict of child_tables -> {child_table_name: child_table_df}
        self.parent_tables = (
            {}
        )  # dict of parent_tables -> {parent_table_name: (parent_table, foreign_key_column)
    ...
    def add_child_table(self, child_table, foreign_key_column):
        ...
        if not isinstance(child_table, RelationalDf):
            raise TypeError(
                f"child_table must be an instance of {RelationalDf.__name__}"
            )
        ...
        if child_table.table_id in self.child_tables:
            raise KeyError(
                f"'{child_table.table_id}' already exists as child table of '{self.table_id}'"
            )
    ...
    def add_parent_table(self, parent_table, foreign_key_column):
        ...
        if not isinstance(parent_table, RelationalDf):
            raise TypeError(
                f"parent_table must be an instance of {RelationalDf.__name__}"
            )
        ...
        if parent_table.table_id in self.parent_tables:
            raise KeyError(
                f"'{parent_table.table_id}' already exists as child table of '{self.table_id}'"
            )
