...
usage_windows = [
    {"id": 1, "appliance_id": 1, "start": 14, "end": 16},
    {"id": 2, "appliance_id": 1, "start": 20, "end": 24},
]


users = RelationalDf("users", "users", pd.DataFrame(users))
appliances = RelationalDf("appliances", "appliances", pd.DataFrame(appliances))
usage_windows = RelationalDf(
    "usage_windows", "usage_windows", pd.DataFrame(usage_windows)
)
...
first_table = users
display_tables_dict = {
    users.table_id: users,
    appliances.table_id: appliances,
    usage_windows.table_id: usage_windows,
}
