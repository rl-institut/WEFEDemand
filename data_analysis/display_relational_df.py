...
@callback(
    ...
    [
        ...
        Input(
            {"type": "close_table_button", "table_id": ALL, "table_number": ALL},
            "n_clicks",
        ),
        Input(
            {"type": "add_row_button", "table_id": ALL, "table_number": ALL}, "n_clicks"
        ),
    ],
    [
...
def update_tables(
    ...
    if element_clicked["type"] == "table":
        ...
        test = table_cell_clicked(
            active_cell, displayed_tables_list, current_tables_view
        ) + (displayed_tables_data,)

        return table_cell_clicked(
            active_cell, displayed_tables_list, current_tables_view
        ) + (displayed_tables_data,)
        ...
        return close_table_button_clicked(
            close_table_button_clicks, displayed_tables_list, current_tables_view
        ) + (displayed_tables_data,)
...
def table_cell_clicked(active_cell, displayed_tables, current_tables_view):
    ...
    if active_cell[table_clicked["table_number"]]["column_id"].startswith("!child_"):
        ...
        query_col = (
            display_tables_dict[table_clicked["table_id"]]
            .child_tables[child_table_id]
            .parent_tables[table_clicked["table_id"]][1]
        )
        ...
        if child_table_id not in displayed_tables:
            ...
            new_table = display_table(
                relational_df=display_tables_dict[
                    table_clicked["table_id"]
                ].child_tables[child_table_id],
                table_number=len(
                    current_tables_view
                ),  # position of new table is at the end of current tables view
                query_col=query_col,
                query_id=query_id,
            )
            ...
            if (
                displayed_tables.get(child_table_id)[1]
                == active_cell[table_clicked["table_number"]]["row_id"]
            ):
...
def close_table_button_clicked(n, displayed_tables, current_tables_view):
    ...
    displayed_tables = dict(
        list(displayed_tables.items())[0 : button_clicked["table_number"]]
    )
...
def add_table_row(
    ...
    for col in columns:
        if col["id"].startswith("!child_"):  # for button column
            ...
            # TODO
            new_row[col["id"]] = (
                display_tables_dict[button_clicked["table_id"]].df["id"].max() + 1
            )
        elif (
            col["id"] == displayed_tables[button_clicked["table_id"]][0]
        ):  # for query_col
            # Enter query_id
            new_row[col["id"]] = displayed_tables[button_clicked["table_id"]][
                1
            ]  # value is query_id
        else:
            new_row[col["id"]] = ""
...
def display_table(relational_df, table_number, query_col=None, query_id=None):
    ...
    new_table_row = html.Div(
        ...
        id=relational_df.table_id,
        className="table_row_wrapper",
    )

    return new_table_row


from data_analysis.data_analysis_test import (
    display_tables_dict,
)  # dict containing all tables to be displayed
